...
- **Auto-save:** changes to `topo`, `positions`, `edgeOffsets`, `processSetKey`, or `winCondition` start a single 800 ms debounce timer; the first run after load is skipped. For new topologies the first auto-save calls `POST /api/topologies/new` and navigates to `/topology/:id/edit` (replace history). For existing topologies it calls `POST /api/topology/:id` with the full topology, including `editor_edge_offsets` and `editor_process_set`. A "Saving…" / "Saved" indicator appears in the sidebar bottom.
...
- **Topology storage:** `topologies/<id>.json` — same JSON schema as game `round_def` plus `editor_positions: { "<letter>": {x, y} }`, `editor_edge_offsets: { "<A-B>": {ox, oy} }`, `editor_process_set: "<key>"`, and `editor_win_condition: {brown, red, purple}`. `_positions` must be `editor_positions` — Pydantic v2 treats underscore-prefixed fields as private. All editor state (including edge curves and process-set selection) lives in this git-tracked file — there is no separate local layout file for topologies, so cloning the repo reproduces a topology exactly as designed. (Round replay layouts are still separate — see `layout/<round_id>.json` above.)
