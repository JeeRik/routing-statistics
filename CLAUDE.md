...
- **Distribution layer:** a 5×2 color picker (materials A–I in production-chain order) lets you select one material. Each node gains a summary row between Storage and Cargo: producers show `⚙ +produced` (grey cog icon), consumers show `🚚 -delivered / ⛔ -taxed` (azure truck / red stop icons), nodes that only taxed goods show `⛔ -taxed`, others show an empty spacer. Numbers are in the material's color. Each edge also gets an animated highlight strip (same SMIL animation as Traffic) in the material's color at 40 %→80 %→40 % opacity, width scaled to the busiest edge for that material. Fetches `/distribution` (node stats) and `/traffic` (edge stats) on every time or material change.
